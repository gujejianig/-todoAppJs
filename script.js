...
const showTask = () => {
    ...
    counter = initialTaskData.length
    ...
    // indexcontrol უნდა გაიზაროდს 5 ით, როდესაც index 5-ს მიაღწევს/
    // index უნდა იზრდებოდეს initialTaskData.length - ის მეშვეობით (მასზე უნდა იყოს დამოკიდებული)
    // index როცა მიაღწევს 5 მდე, უნდა განულდეს//////

    // როგორ გავიგოთ, როდის უნდა განულდეს ინდექსი? ----->  arr.length = 5, 10 15 <-----



    if (initialTaskData.length >=0) {
        taskList.innerHTML =
            initialTaskData.slice(indexControl, indexControl + 5)
                ?.map((task, index) => {
